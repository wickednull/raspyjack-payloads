#!/usr/bin/env python3
"""
RaspyJack *payload* – **Evil Twin (PayPal Phishing)**
===================================================
This payload sets up an Evil Twin attack to phish for PayPal credentials.
It creates a fake Wi-Fi access point, redirects all DNS requests to a local
web server hosting a fake PayPal login page, and captures any entered
credentials.

Features:
- Creates a fake Wi-Fi AP with a configurable SSID and channel.
- Uses `hostapd` and `dnsmasq` for AP and DHCP/DNS services.
- Hosts a customizable phishing portal (default PayPal login page).
...
def start_attack():
    ...
    if subprocess.run("which hostapd", shell=True, capture_output=True).returncode != 0:
        ...
        time.sleep(3)
        return False, "hostapd not found"
