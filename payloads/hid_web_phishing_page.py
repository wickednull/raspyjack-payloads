#!/usr/bin/env python3
"""
RaspyJack *payload* – **HID Attack: Open Phishing Page**
=========================================================
...
PHISHING_URL = "http://10.0.0.1" # Will be configurable
...
# --- Globals & Shutdown ---
running = True
current_url_input = PHISHING_URL # For URL input
...
def draw_ui(screen_state="main"):
    ...
    d.text((5, 5), "Open Phishing Page", font=FONT_TITLE, fill="#00FF00")
    d.line([(0, 22), (128, 22)], fill="#00FF00", width=1)
    
    if screen_state == "main":
        d.text((5, 30), "Phishing URL:", font=FONT, fill="white")
        d.text((5, 45), PHISHING_URL[:16] + "...", font=FONT_TITLE, fill="yellow")
        d.text((5, 115), "OK=Start | KEY1=Edit URL | KEY3=Exit", font=FONT, fill="cyan")
    elif screen_state == "url_input":
        d.text((5, 30), "Enter Phishing URL:", font=FONT, fill="white")
        ...
        d.text((5, 70), f"URL: {PHISHING_URL[:16]}...", font=FONT, fill="white")
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID Open Phishing Page.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        while running:
            if current_screen == "main":
                ...
                if GPIO.input(PINS["KEY1"]) == 0: # Edit Phishing URL
                    current_url_input = PHISHING_URL
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID Open Phishing Page payload finished.")
