...
"""
RaspyJack *payload* – **HID Attack: Open Application (macOS)**
===============================================================
A simple HID attack that opens a specified application on macOS using
the `open` command.
"""
...
# --- CONFIGURATION ---
APP_TO_OPEN = "Calculator" # Will be configurable
...
# --- Globals & Shutdown ---
running = True
current_app_input = APP_TO_OPEN # For app name input
app_input_cursor_pos = 0
...
def draw_ui(screen_state="main"):
    if not HARDWARE_LIBS_AVAILABLE:
        ...
        if screen_state == "main":
            print(f"App to Open: {APP_TO_OPEN}")
        return

    img = Image.new("RGB", (128, 128), "black")
    d = ImageDraw.Draw(img)
    d.text((5, 5), "macOS Open App", font=FONT_TITLE, fill="#00FF00")
    d.line([(0, 22), (128, 22)], fill="#00FF00", width=1)
    
    if screen_state == "main":
        d.text((5, 30), "App to Open:", font=FONT, fill="white")
        d.text((5, 45), APP_TO_OPEN[:16], font=FONT_TITLE, fill="yellow")
        d.text((5, 115), "OK=Start | KEY1=Edit App | KEY3=Exit", font=FONT, fill="cyan")
    elif screen_state == "app_input":
        d.text((5, 30), "Enter App Name:", font=FONT, fill="white")
        display_app = list(current_app_input)
        if app_input_cursor_pos < len(display_app):
            display_app[app_input_cursor_pos] = '_'
        d.text((5, 50), "".join(display_app[:16]), font=FONT_TITLE, fill="yellow")
        d.text((5, 115), "UP/DOWN=Char | LEFT/RIGHT=Move | OK=Confirm", font=FONT, fill="cyan")
    elif screen_state == "attacking":
        d.text((5, 50), "Injecting Commands...", font=FONT_TITLE, fill="yellow")
        d.text((5, 70), f"App: {APP_TO_OPEN[:16]}", font=FONT, fill="white")
...
def handle_text_input_logic(initial_text, screen_state_name, char_set):
    global current_app_input, app_input_cursor_pos
    
    current_input_ref = current_app_input
    cursor_pos_ref = app_input_cursor_pos
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID macOS Open App.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        while running:
            if current_screen == "main":
                ...
                if GPIO.input(PINS["KEY1"]) == 0: # Edit App Name
                    current_app_input = APP_TO_OPEN
                    current_screen = "app_input"
                    time.sleep(0.3) # Debounce
            
            elif current_screen == "app_input":
                char_set = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .-" # Common app name chars
                new_app = handle_text_input_logic(current_app_input, "app_input", char_set)
                if new_app:
                    APP_TO_OPEN = new_app
                current_screen = "main"
                time.sleep(0.3) # Debounce
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID macOS Open App payload finished.")
