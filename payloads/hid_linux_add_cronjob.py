#!/usr/bin/env python3
"""
RaspyJack *payload* – **HID Attack: Add Cronjob (Linux)**
==========================================================
A HID attack that adds a cronjob to the user's crontab for persistence.
This example sets up a cronjob to execute a reverse shell every minute.

**NOTE:** This is a noisy persistence method and can be easily detected.
"""
...
def draw_ui(screen_state="main"):
    ...
    img = Image.new("RGB", (128, 128), "black")
    d = ImageDraw.Draw(img)
    d.text((5, 5), "Add Cronjob", font=FONT_TITLE, fill="#00FF00")
    ...
    if screen_state == "main":
        ...
        d.text((5, 50), "Injecting Cronjob...", font=FONT_TITLE, fill="yellow")
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID Cronjob.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID Cronjob payload finished.")
