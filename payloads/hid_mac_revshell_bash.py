...
def draw_ui(screen_state="main"):
    ...
    d.text((5, 5), "macOS Bash Revshell", font=FONT_TITLE, fill="#00FF00")
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID macOS Bash Revshell.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID macOS Bash Revshell payload finished.")
