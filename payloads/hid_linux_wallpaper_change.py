...
# --- CONFIGURATION ---
IMAGE_URL = "https://i.imgur.com/w4n2y5V.jpeg" # Will be configurable
IMAGE_PATH = "/tmp/wallpaper.jpg" # Will be configurable
...
# --- Globals & Shutdown ---
running = True
current_url_input = IMAGE_URL # For URL input
url_input_cursor_pos = 0
current_path_input = IMAGE_PATH # For path input
path_input_cursor_pos = 0
...
def draw_ui(screen_state="main"):
    ...
    d.text((5, 5), "Linux Wallpaper", font=FONT_TITLE, fill="#00FF00")
    d.line([(0, 22), (128, 22)], fill="#00FF00", width=1)
    
    if screen_state == "main":
        d.text((5, 30), "Image URL:", font=FONT, fill="white")
        d.text((5, 45), IMAGE_URL[:16] + "...", font=FONT_TITLE, fill="yellow")
        d.text((5, 65), "Image Path:", font=FONT, fill="white")
        d.text((5, 80), IMAGE_PATH[:16] + "...", font=FONT_TITLE, fill="yellow")
        d.text((5, 115), "OK=Start | KEY1=Edit URL | KEY2=Edit Path | KEY3=Exit", font=FONT, fill="cyan")
    elif screen_state == "url_input":
        d.text((5, 30), "Enter Image URL:", font=FONT, fill="white")
    ...
    elif screen_state == "path_input":
        d.text((5, 30), "Enter Image Path:", font=FONT, fill="white")
        display_path = list(current_path_input)
        if path_input_cursor_pos < len(display_path):
            display_path[path_input_cursor_pos] = '_'
        ...
        d.text((5, 70), f"URL: {IMAGE_URL[:16]}...", font=FONT, fill="white")
...
def handle_text_input_logic(initial_text, screen_state_name, char_set):
    global current_url_input, url_input_cursor_pos, current_path_input, path_input_cursor_pos
    
    if screen_state_name == "url_input":
        current_input_ref = current_url_input
        cursor_pos_ref = url_input_cursor_pos
    else: # path_input
        current_input_ref = current_path_input
        cursor_pos_ref = path_input_cursor_pos
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID Wallpaper Change.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        while running:
            if current_screen == "main":
                ...
                if GPIO.input(PINS["KEY1"]) == 0: # Edit Image URL
                    current_url_input = IMAGE_URL
                    current_screen = "url_input"
                    time.sleep(0.3) # Debounce
                
                if GPIO.input(PINS["KEY2"]) == 0: # Edit Image Path
                    current_path_input = IMAGE_PATH
                    current_screen = "path_input"
                    time.sleep(0.3) # Debounce
            
            elif current_screen == "url_input":
                char_set = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._~:/?#[]@!{new_string}'()*+,;=%%" # Common URL chars
            ...
            elif current_screen == "path_input":
                char_set = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789/._-" # Common path chars
                new_path = handle_text_input_logic(current_path_input, "path_input", char_set)
                if new_path:
                    IMAGE_PATH = new_path
                current_screen = "main"
                time.sleep(0.3) # Debounce
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID Wallpaper Change payload finished.")
