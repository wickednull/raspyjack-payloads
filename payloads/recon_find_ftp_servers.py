...
def draw_ui():
    ...
    with ui_lock:
        if "Scanning" in status_msg or "Press" in status_msg:
            ...
            d.text((5, 25), f"Servers Found: {len(ftp_servers)}", font=FONT, fill="yellow")
            start_index = max(0, selected_index - 4)
            end_index = min(len(ftp_servers), start_index + 8)
            y_pos = 40
...
# --- Scanner ---
def run_scan():
    global ftp_servers, status_msg, selected_index
    with ui_lock:
