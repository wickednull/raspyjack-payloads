...
"""
RaspyJack *payload* – **Recon: Find FTP Servers**
==================================================
A simple reconnaissance payload that scans the local network to find
hosts with the FTP port (21) open.
"""
...
# --- CONFIGURATION ---
FTP_PORT = 21 # Will be configurable
...
current_port_input = str(FTP_PORT) # For port input
...
# --- Main Loop ---
if not HARDWARE_LIBS_AVAILABLE:
    print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run Find FTP Servers.", file=sys.stderr)
    sys.exit(1)
